!
! RADMC-3D versio 0.41 main.f90 patched version
!
! Makes setting nphot_therm possible when run as 
! child process using the pipe.
!
! Laszlo Szucs (laszlo.szucs@mpe.mpg.de)
!
program radmc3d
  use amr_module
  use rtglobal_module
...
!        rt_incl_lines = .false.
     ...
     elseif(buffer(1:11).eq.'nphot_therm') then
        !
        ! Set the nr of photons for the thermal monte carlo
        !
        if(iarg.gt.numarg) then
